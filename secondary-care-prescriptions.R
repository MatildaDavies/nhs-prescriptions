...
message("Step 1: Fetching available monthly file paths via the Open Data Portal API...")
...
message("\nStep 2: Commencing download and text-filtering sweep...")
for (file_url in scmd_csv_urls) {
  ...
  # Stream input utilizing 'cols_only' to prevent R workspace RAM crashes
  monthly_raw <- tryCatch({
    ...
    message("   Warning: Parse layout mismatch. Skipping month.")
    NULL
  })
  ...
  # If matching records were pulled, safely aggregate costs to product level
  if (nrow(monthly_matched) > 0) {
    ...
    message("   Success: Logged ", sum(monthly_aggregated$RECORDS_COUNT), " row entries.")
  } else {
...
message("\nStep 3: Merging data blocks into unified timeline dataset...")
if (length(collated_scmd_list) > 0) {
  ...
  message("Compiled file exported to path: ", getwd(), "/", output_filename)
